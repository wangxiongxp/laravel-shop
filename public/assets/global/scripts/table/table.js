...
Table.prototype = {
	...
	configPageInfo: function(expr) {
        ...
        var pageNo = this.config.pageNo ;
        var pageCount = this.config.pageCount ;
        var totalCount = this.config.totalCount ;

		var html = '';
		if (this.config.totalCount == 0) {
			html = this.messages['page.empty'];
		} else {
			html = this.messages['page.info'].call(this, pageNo, pageCount, totalCount);
		}
		$(expr).html(html);
	},
	...
	messages: {
		'page.info': function() {
			return "第 "+ arguments[0] +" 页 ( 共 "+ arguments[1] +" 页，"+ arguments[2] +" 条记录 )";
		},
		'page.empty': '第 1 页 ( 共 0 条记录 )',
