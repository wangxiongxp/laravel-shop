                ...
                <div class="portlet light bordered">
                    ...
                    <div class="portlet-title">
                        <div class="caption font-dark">
                            ...
                            <span class="caption-subject bold uppercase"> 编辑产品</span>
                        </div>
                    </div>
                    ...
                    <div class="portlet-body form">
                        ...
                        <form role="form" class="form-horizontal" method="post" id="EditForm" action="/admin/product/update">
                            <div class="tab-content">
                                <div class="tab-pane active" id="tab_general">
                                    <div class="form-body" style="padding-top:30px;">
                                        <div class="form-group">
                                            <input type="hidden" id="product_id" name="product_id" value="{{$product->product_id}}" >
                                            <input type="hidden" id="product_cat_id" name="product_cat_id" value="{{$product->product_cat_id}}" >
                                            <label class="col-md-2 control-label">所属分类<span class="required" >*</span></label>
                                            <div class="col-md-4">
                                                <p class="form-control-static">{{ $product_cat->cat_title }}</p>
                                            </div>
                                        </div>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="col-md-10">
                                                <input type="text" id="product_title" name="product_title" value="{{ $product->product_title }}" placeholder="" class="form-control">
                                            </div>
                                        </div>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="col-md-10">
                                                <input type="text" id="product_sub_title" name="product_sub_title" value="{{ $product->product_sub_title }}" placeholder="" class="form-control">
                                            </div>
                                        </div>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="col-md-10">
                                                <input type="text" id="product_price" name="product_price" value="{{ $product->product_price }}" placeholder="" class="form-control">
                                            </div>
                                        </div>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="col-md-10">
                                                <input type="text" id="product_qty" name="product_qty" value="{{ $product->product_qty }}" placeholder="" class="form-control">
                                            </div>
                                        </div>
                                        ...
                                        <div class="image-list" id="filelist">
                                            @foreach($product_images as $image)
                                                <div class="image-item" >
                                                    <a id="file-{{$image->image_id}}">
                                                        <img id="img-{{$image->image_id}}'" src="{{$image->image_url}}"/>
                                                        <input type="hidden" name="image_url[]" value="{{$image->image_url}}">
                                                        <input type="hidden" name="image_name[]" value="{{$image->image_title}}">
                                                    </a>
                                                </div>
                                            @endforeach
...
@section('foot_script')
        ...
        <div class="form-group prop_item" :dataVal="attr.attr_id" :dataName="attr.attr_name" :dataType="attr.attr_type">
            ...
            <div class="col-md-10">
                <div v-if="attr.attr_type == 'UT_INPUT'">
                    <input type="text" name="" value="" class="form-control ut_input">
                </div>
                <div v-else-if="attr.attr_type == 'UT_SELECT'">
                    <select class="form-control ut_select" id="parent_id" name="parent_id">
                ...
                <div v-else-if="attr.attr_type == 'UT_CHECKBOX'">
                    <div class="checkbox-list">
                        <label class="checkbox-inline" v-for="option in attr.options">
                            <input type="checkbox" class="ut_checkbox" :id="'pid_' + attr.attr_id + '_' + option.option_id"
                                   :name="'pid_' + attr.attr_id + '_' + option.option_id" :value="option.option_id" :dataName="option.option_name"> @{{option.option_name}} </label>
                    </div>
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        var vm = new Vue({
            ...
            mounted: function () {
                ...
                $.ajax({
                    type: 'GET',
                    url:"/admin/productCatAttr/getProductCatAttrByCatId?cat_id={{$product->product_cat_id}}",
                    dataType: "json",
    ...
    <script>
        ...
        function initAttr() {
            ...
            var attrs_json = '{!! $product_attr !!}';
            if (attrs_json == '')return;
            var attrs = JSON.parse(attrs_json);
            $.each(attrs, function (i, item) {
                var $prop_item = $("div[dataVal=" + item.attr_id + "].prop_item");
                if (item.attr_type == 'UT_INPUT') {
                    $prop_item.find("input.ut_input").val(item.attr_value);
                } else if (item.attr_type == 'UT_CHECKBOX') {
                    var b = $prop_item.find("input.ut_checkbox[name=pid_" + item.attr_id + '_' + item.option_id + ']');
                    b.attr('checked', true);
                    b.attr("dataName", item.option_value);
                    b.next('input').val(item.option_value);
                } else if (item.attr_type == 'UT_SELECT') {
                    $prop_item.find("select.ut_select option").each(function () {
                        if (this.value == item.option_id) {
    ...
    <script>
        ...
        $(function(){
            ...
            SetRadioSelected('product_status','{{$product->product_status}}');
